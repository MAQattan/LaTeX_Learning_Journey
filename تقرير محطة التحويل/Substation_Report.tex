...
\setbeamertemplate{headline}{
  ...
  \begin{picture}(0,0)
  \put(270,-25){\includegraphics[width=2.3cm]{logo.png}}
  \end{picture}
  \begin{picture}(0,0)
    \put(30,20){%
...
\begin{frame}{ساحة التوزيع والمعدات الخارجية \en{(Switchyard)}}
    \fontsize{10pt}{12pt}\selectfont
  \textbf{مفهوم الـ \en{Switchyard}:} تُسمى الساحة الخارجية بهذا الاسم لأنها محطة "هوائية"، حيث يُستخدم الهواء الطبيعي كوسط عازل بين الفازات والمعدات، وتُصمم المسافات بين الناقلات بناءً على مستوى الجهد (مثلاً جهد \en{66} كيلو فولت يتطلب مسافة عازلة تبلغ حوالي \en{16} سم).

  \vspace{10pt}
  \begin{columns}[onlytextwidth]
    \begin{column}{0.4\textwidth}
      \textbf{خطوط الدخول \en{(Incomers)}:} تستقبل الساحة خطوطاً بجهد \en{66} كيلو فولت قادمة من محطات أخرى مثل (محطة الشركـة ومحطة الجامعة)، 
...
\begin{frame}{ساحة التوزيع والمعدات الخارجية \en{(Switchyard)}}
  \textbf{أجهزة القياس والتحويل \en{(Instrument Transformers)}:}\\[15pt]
  \begin{itemize}
    \begin{columns}[onlytextwidth]
      \begin{column}{0.4\textwidth}
...
\begin{frame}{ساحة التوزيع والمعدات الخارجية \en{(Switchyard)}}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \begin{itemize}
          \item \textbf{القواطع الآلية \en{(Circuit Breakers)}:} وهي قواطع تعمل بغاز \en{SF6} (سداسي فلوريد الكبريت) لإطفاء القوس الكهربائي، وتُستخدم لفصل الخطوط أو المحولات آلياً عند حدوث أعطال.\\[5pt]
          \item \textbf{السكاكين \en{(Disconnectors)}:} تُستخدم للفصل الميكانيكي الظاهر للعين لتأمين مسافة أمان أثناء الصيانة، وتتحرك يمين يسار لفتح الدارة.\\
      \end{itemize}
    \end{column}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{columns}
        \begin{column}{0.5\textwidth}
          ...
          {\footnotesize\scriptsize قاطع آلي \en{Circuit Breaker} 66 ك.ف}
        \end{column}
          \hspace{-0.5cm}
        \begin{column}{0.5\textwidth}
          \centering
          \includegraphics[width=3cm, height=4.2cm]{Ph6.jpg}\\
          {\footnotesize\scriptsize قاطع فصل هوائي \en{Disconnector}}
        \end{column}
      \end{columns}}
    \end{column}
  \end{columns}
    ...
    \vspace{5pt}
  \textbf{منظومة الحماية من الصواعق:} توجد "قرون التفريغ" على العوازل \en{(Bushings)}، وهي فجوات هوائية تحمي العازل من الجهود الزائدة (الناجمة عن الصواعق أو المناورات) عبر تفريغ الشحنة إلى الأرض.
\end{frame}
...
\begin{frame}{المحول الرئيسي (القلب النابض)}
  ...
  يحتوي المحول على كميات ضخمة من \textbf{زيت العزل المعدني} (تصل إلى حوالي \en{40} برميل في المحولات الكبيرة)، وهو زيت خاص يختلف تماماً عن الزيوت العادية.\\
  يتم أخذ عينات دورية من الزيت لفحص "العازلية"، وفي حال تراجع جودته يتم إجراء عملية \textbf{تصفية أو تكرير} لاستعادة خواصه العازلة.\\
\end{frame}
...
\begin{frame}{المحول الرئيسي (القلب النابض)}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \textbf{نظام التبريد \en{(Cooling System)}:}\\
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{columns}
        \begin{column}{0.5\textwidth}
          \centering
          \includegraphics[width=2.7cm, height=2.7cm]{Ph8.png}
          {\footnotesize\scriptsize مروح من بنية المحول (مدمجة)}
        \end{column}
        \hspace{-0.5cm}
        \begin{column}{0.5\textwidth}
          \centering
          \includegraphics[width=2.7cm, height=2.7cm]{Ph9.jpg}
          {\footnotesize\scriptsize مراوح من خارجية (مضافة)}
        \end{column}
      \end{columns}
    \end{column}
  \end{columns}
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}
      \begin{columns}
        \begin{column}{0.5\textwidth}
          \centering
          \includegraphics[width=2.5cm, height=3.5cm]{Ph10.png}\\
          {\footnotesize\fontsize{7pt}{9pt}\selectfont المقاييس التي تقيس درجة الحرارة}
        \end{column}
        \begin{column}{0.5\textwidth}
          \centering
          \includegraphics[width=2.5cm, height=3.5cm]{Ph11.png}\\
          {\footnotesize\fontsize{7pt}{9pt}\selectfont دارة التحكم \en{Control Circuit}}
        \end{column}
      \end{columns}
    \end{column}
        % \hspace{-0.5cm}
    \begin{column}{0.45\textwidth}
      \textbf{أجهزة المراقبة والقياس:}\\
      توجد حساسات ومقاييس حرارة على جسم المحول، يتم ضبط الحساسات لتعطي إشارات إنذار \en{(Alarm)} عند درجة \en{70} مئوية، وأمر فصل \en{(Trip)} عند درجة \en{90} مئوية لحماية الملفات من التلف.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{أنظمة حماية المحول (الميكانيكية والغازية)}
  \fontsize{10pt}{12pt}\selectfont
   \textbf{حماية البوخلز \en{(Buchholz Relay)}:}\\
...
آلية العمل: تحتوي على "فواشات" تعطي أمر فصل \en{(Trip)} للمحول في حالتين:
\begin{itemize}
  \item تولد غازات: نتيجة حدوث قصر \en{(Short Circuit)} بين الملفات يؤدي لغليان الزيت وتبخره.
  \item نقص مستوى الزيت: إذا انخفض مستوى الزيت دون حد الأمان.
\end{itemize}
\vspace{10pt}
\begin{columns}
  \begin{column}{0.55\textwidth}
    \textbf{خزان الزيت الاحتياطي \en{(Conservator Tank)}:}\\
    يوجد خزان علوي يحافظ دائماً على مستوى زيت (بحدود الثلث أو الوسط) لتعويض تمدد الزيت عند الحرارة وتقلصه عند البرودة.\\
    ينقسم الخزان أحياناً إلى حجرتين؛ واحدة للمحول الرئيسي والأخرى لمنظم الجهد \en{(Tap Changer)} لضمان استقلالية الزيت في كل منهما.
  \end{column}
...
\begin{frame}{أنظمة حماية المحول (الميكانيكية والغازية)}
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \textbf{مجفف الهواء \en{(Silica Gel Breather)}:}\\[5pt]
    ...
    \begin{column}{0.4\textwidth}
      ...
      {\footnotesize\scriptsize مجفف الهواء \en{(Silica Gel Breather)}}
    \end{column}
  \end{columns}
  \textbf{صمام الضغط الزائد \en{(Pressure Relief Valve)}:}\\
    يعمل تماماً مثل "صمام طنجرة الضغط"؛ ففي حال حدوث عطل انفجاري مفاجئ يولد ضغطاً هائلاً، يفتح الصمام لتفريغ الضغط وحماية جسم المحول \en{(Structure)} من الانفجار الكامل.
\end{frame}

\begin{frame}{منظم الجهد تحت الحمل \en{(On-Load Tap Changer - OLTC)}}
  \metroset{block=fill}
  \begin{block}{مفهوم التنظيم تحت الحمل \en{(On-Load)}:}
      \vspace{6pt}
    \begin{columns}
      \begin{column}{0.5\textwidth}
         يتميز هذا المنظم بقدرته على تغيير نسبة التحويل \en{(Voltage Ratio)} دون الحاجة لفصل التيار الكهربائي عن المشتركين، وهو ما يجعله مختلفاً عن محولات التوزيع الصغيرة ($20/0.4$ ك.ف) التي تتطلب فصلاً كاملاً للمحول عند تغيير الوضعيات.
      \end{column}
      \hspace{-1.6cm}
      \begin{column}{0.4\textwidth}
        \centering
        \includegraphics[width=3.2cm, height=3.8cm]{Ph13.png}\\
        {\footnotesize\scriptsize الشكل الخارجي لمنظم الجهد تحت الحمل \en{OLTC}}
      \end{column}
    \end{columns}
  \end{block}
...
\begin{frame}{منظم الجهد تحت الحمل \en{(On-Load Tap Changer - OLTC)}}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item \textbf{عدد الوضعيات:} يحتوي المنظم على \en{19} وضعية \en{(Steps)} تتيح رفع أو خفض الجهد بدقة عالية.\\[15pt]
    ...
    \begin{column}{0.45\textwidth}
      \centering
      \includegraphics[width=1\textwidth]{Ph14.png}\\
      {\footnotesize\scriptsize منظم الجهد تحت الحمل \en{OLTC} من الداخل}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{منظم الجهد تحت الحمل \en{(On-Load Tap Changer - OLTC)}}
  \begin{itemize}
    \item \textbf{الجزء المتحرك \en{(Diverter Switch)}:} هو الجزء الداخلي الذي يقوم فعلياً بعملية التبديل بين مخارج ملفات التنظيم المجهزة مسبقاً داخل المحول.\\[10pt]
...
\begin{frame}{نظام التأريض ومحول الخدمات \en{(Zig-Zag)}}
\begin{columns}
  \begin{column}{0.45\textwidth}
    بما أن الملف الثانوي للمحول الرئيسي موصول على شكل مثلثي، فإنه يفتقر لـ نقطة حيادي تكتمل عبرها الدارة عند حدوث قصر، لذا يتم استخدام \textbf{محول تأريض مساعد} (ذو توصيلة هندسية خاصة "زكزاك \en{(Zig-Zag)})" لخلق نقطة حيادي، وبهذا الطريقة تتمكن الحمايات من كشف التسريب وفصل المحول في حينها.\\
...
\begin{frame}{  وتوزيع الأحمال \en{(Bus Coupling)}}
\begin{columns}
  \begin{column}{0.5\textwidth}
  ...
  \textbf{\small عملية الكوبلاج \en{(Bus Coupling)}:} هي عملية ربط البارات \en{(Busbars)} لضمان استمرار التغذية عند خروج محول للصيانة، أو لـ موازنة الأحمال بين المحولات.\\
  لا يمكن إتمام الكوبلاج إلا بتحقيق "التوافق" في ثلاثة شروط: تساوي التوتر، توافق التردد، وتطابق فرق الصفحة.
  \end{column}
  \hspace{-1cm}
  \begin{column}{0.45\textwidth}
    \centering
    \includegraphics[width=0.9\textwidth]{Ph17.png}
  \end{column}
\end{columns}
\end{frame}
